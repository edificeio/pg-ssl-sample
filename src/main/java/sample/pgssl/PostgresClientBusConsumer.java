...
public class PostgresClientBusConsumer implements IPostgresClient {
    ...
    PostgresClientBusConsumer(final Vertx vertx, final IPostgresClient inner, final String suffix){
        ...
        this.consumer.handler(message -> {
            ...
            LoggerFactory.getLogger(getClass()).info("Receive query postgres for consumer..."+PostgresClientBusHelper.getAddress(suffix));
            LoggerFactory.getLogger(getClass()).info("Receive query postgres..."+Vertx.currentContext().deploymentID());
            if(isQuery(payload)){
